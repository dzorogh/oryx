...
const ERROR_TRANSLATIONS: Array<[RegExp, string]> = [
  ...
  [/Posted reservations cannot be cancelled/i, "Проведённый резерв нельзя отменить. Создайте резерв со снятием в «Свободно»."],
  [/Posted warehouse documents cannot be cancelled/i, "Проведённый складской документ нельзя отменить. Создайте новый документ."],
  [/Posted adjustments cannot be cancelled/i, "Проведённую корректировку нельзя отменить. Создайте новый документ."],
  [/Explanation is required/i, "Укажите объяснение корректировки"],
  [/Adjustment works only with free warehouse stock/i, "Корректировка работает только со свободным остатком на складе"],
  [/Not enough free stock/i, "Недостаточно свободного остатка в выбранном месте"],
  [/Add at least one adjustment line/i, "Добавьте хотя бы одну строку товара"],
  [/Expected a posted adjustment/i, "Ожидалась проведённая корректировка"],
  [/Shipment can only consume stock reserved for a customer order/i, "Отгрузка может списать только резерв заказа клиента."],
  ...
  [/Transfer request key is required/i, "Нужен ключ запроса перемещения"],
  [/Both warehouses are required/i, "Выберите склады отправления и назначения"],
  [/Source and destination warehouses must differ/i, "Склад отправления и склад назначения должны отличаться"],
  [/Add at least one transfer line/i, "Добавьте хотя бы одну строку перемещения"],
  [/Duplicate product/i, "Один товар можно указать только один раз"],
  [/Unknown warehouse/i, "Неизвестный склад"],
  [/Expected sent transfer/i, "Ожидался отправленный документ перемещения"],
