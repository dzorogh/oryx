...
const ERROR_TRANSLATIONS: Array<[RegExp, string]> = [
  ...
  [/Product must match the customer order line/i, "Товар должен совпадать со строкой заказа"],
  [/Use the close operation to finish a production order/i, "Чтобы завершить, нажмите «Close production order»"],
  ...
  [/Cannot reserve more than the open customer order quantity/i, "Нельзя забронировать больше открытого количества заказа"],
  ...
  [/Reservation line must belong to the customer order/i, "Строка бронирования должна относиться к заказу"],
  [/Shipment line must match the single customer order/i, "Строка отгрузки должна относиться к одному заказу"],
