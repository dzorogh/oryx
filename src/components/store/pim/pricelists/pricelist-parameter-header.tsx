...
type PricelistParameterHeaderCellProps = {
  ...
  /** Any parameter header is being dragged — suppress the label tooltip. */
  isDragging: boolean;
  onInsertBefore: () => void;
  onInsertAfter: () => void;
  onEdit: () => void;
      ...
      >
        <div className="min-w-0 flex-1">
          ...
          >
            <TooltipTrigger
              render={
                <span className="inline-block max-w-full truncate align-middle text-xs font-semibold text-foreground" />
              }
            >
              {def.label}
            </TooltipTrigger>
            ...
            >
              ...
              {def.formula ? (
                <span className="font-mono text-background/80">
                  {def.formula}
                </span>
              ) : null}
            </TooltipContent>
