...
Страница **Pricelists** (`/store/pim/pricelists`): редактирование закупочных, дилерских и розничных цен в одной таблице **совместно и в реальном времени** (Yjs + WebSocket). Три типа прайслиста (**Global / Supplier / Dealer**), выбор региона, динамические колонки-параметры с формулами, presence-индикаторы соавторов.

**Источники данных:** регионы, группы, валюты, варианты, `store_product_price` и `store_product_region_status` загружаются из demo Supabase (`loadPricelistDbBootstrap` → `applyPricelistDbBootstrap`). Live-правки в сессии остаются в collab (Yjs) и перекрывают DB-значения; без Supabase UI падает на offline fallback-списки.
...
Страница — полностью клиентская интерактивная вьюха (Yjs-состояние), поэтому **сама владеет состоянием** `scope`/`regionId` и синхронизирует URL через History API (`window.history.replaceState`). `popstate` возвращает состояние при навигации назад/вперёд. Не используем `router.replace` для scope/region.
...
Каждая цена — **одна колонка** с двойным инпутом (исходная валюта + валюта отображения), отдельных `… (USD)` колонок в таблице больше нет.
...
**Колонки наценки** (`kind: "markup"`, считаются из USD-значений, поле `markup: MarkupBasis`). Markup-ячейка несёт единицу **только в ячейке**: показывает округлённое число со знаком `%` (`formatMarkupValue` → `"31%"`), а в заголовке `%` не дублируется. Параметр-колонки остаются без единицы (она в их лейбле):
...
- **Двойная ценовая ячейка** (`pricelist-price-dual-cell.tsx`) — один инпут на цену в виде `сумма │ CNY · USD │ сумма` (пример: `52483 | CNY USD | 7694`). Слева — сумма в исходной валюте; справа — та же цена, сконвертированная в **валюту отображения**. Между ними — компактная кликабельная серединка-чип без шеврона (`pricelist-currency-popover.tsx`): показывает оба кода (`исходная │ отображение`) фиксированной ширины (без сдвига при смене валюты) и по клику открывает попавер с двумя сетками чипов — **Price currency** (валюта цены, per-cell) и **Display currency** (валюта отображения, глобально на все строки). При наведении на чип показывается тултип, поясняющий, какая валюта где и что её можно сменить по клику. Хранится **только** цена в исходной валюте; правая половина считается на фронте (`convertAmount` по статичному курсу `CURRENCY_USD_RATE`, demo-константа). Правка правой половины конвертирует ввод обратно в исходную валюту (`convertAmount`, округление до целого) и пишет в тот же cell-id, что и левая — обе половины синхронизируются мгновенно. Можно править цену в любой из двух валют независимо. В `dealer` scope суммы read-only (статичный текст `1,000 CNY │ 7,694 USD`, `formatMoney` + `formatConvertedValue`), а серединка в этом режиме — **просто разделитель** (тонкая вертикальная линия, без кнопки и попавера): валюту отображения здесь меняют из тулбара. Presence — на одном cell-id (вся «пилюля» подсвечивается при редактировании).
- **Валюта отображения** глобальна для всего списка и хранится как личная настройка вида в `localStorage` (`store-pricelists-display-currency`, дефолт `USD`), хук `use-pricelist-display-currency.ts`. Менять её можно двумя способами: компактным селектором в тулбаре (иконка `Coins`, `aria-label="Display currency"`, тултип-пояснение, стоит справа от селектора региона) или из попавера любой редактируемой ценовой ячейки (в read-only `dealer` попавера нет — только тулбар). Смена мгновенно пересчитывает правую половину во всех ячейках и раскрытиях, а также управляет валютой колонки-конвертации в Excel-экспорте (см. «Экспорт»).
...
> Правая (display) половина ячейки **не** идёт через бэкенд: это редактируемая конвертация на фронте (`convertAmount` в выбранную валюту отображения), обновляется мгновенно. Внутри бэкенд-наценок USD всё ещё считается на лету из исходной цены (`usdFor` в `pricelist-recalc.ts`), но как отдельный target USD больше не материализуется.
...
> Изменение одного входа пересчитывает все зависимые targets: например, правка `Plant Price` (в любой из двух половин двойной ячейки) мгновенно обновляет display-половину на фронте **и** триггерит пересчёт `Global Markup` через бэкенд. Правка base параметра `Total Expenses` пересчитывает унаследованные ячейки колонки и `Dealer Markup w/ Expenses`.
...
- Заголовок **Pricelists** + подзаголовок «Review and update product prices across regions, compare them side by side, and collaborate with your team in real time.»
- Справа — presence-индикатор соавторов
- Строка управления (`pricelists-toolbar.tsx`) рассчитана на одну строку на десктопе: переключатель scope, селектор региона (для supplier/dealer), компактный селектор валюты отображения (сразу справа от региона, с тултипом), поиск, фильтр по категории и три иконочные кнопки `Filters` / `Columns` / `Export` (только иконка + тултип). На мобильном раскладка переносится: scope-переключатель, ниже регион + валюта, затем поиск + категория, и отдельный ряд иконок.
...
Кнопка **Export** в toolbar выгружает текущий прайслист в `.xlsx` целиком на клиенте (без серверного эндпоинта). Генерация — через `write-excel-file` (импорт `write-excel-file/browser`, библиотека грузится лениво внутри обработчика).
...
- **Колонки** — только видимые, в том же порядке, что и в таблице (`leadingColumns + parameters.visibleColumns + trailingColumns`). При сборке каждая двойная ценовая колонка **разворачивается в две**: цена в исходной валюте + отдельная колонка-конвертация в **выбранной валюте отображения** — `… (USD)`, `… (EUR)` и т.д. (`expandColumnForExport` в `assembleExportColumns`, `displayCurrency` пробрасывается из страницы). В таблице это одна ячейка, а для Excel удобнее иметь обе колонки. Наценки при этом всегда считаются из USD (валюта-независимое отношение) и от выбора не зависят.
...
src/components/store/pim/pricelists/
  ...
  pricelist-price-dual-cell.tsx              # двойная ячейка цены: исходная валюта + валюта отображения
...
- **URL через History API** — клиентская страница сама владеет состоянием scope/region; не использовать `router.replace` для них.
...
- **Курсы валют статичны** (demo-константа). Цена — одна двойная ячейка (исходная валюта + валюта отображения): хранится только цена в исходной валюте, правая половина конвертируется на фронте (`convertAmount`) и пишет обратно в тот же cell-id. Валюта отображения глобальна (`localStorage`, дефолт `USD`), меняется из тулбара или из попавера ячейки. Excel-экспорт разворачивает колонку в две (исходная + конвертация в текущую валюту отображения).
