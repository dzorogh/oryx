...
| Scope | Колонки |
|-------|---------|
| `global` | `Name`, `Plant Price`, `Plant Price (USD)`, `Dealer Status` |
| `supplier` | `Name`, `Plant Price`, `Plant Price (USD)`, `Dealer Price`, `Dealer Price (USD)`, `Global Markup (%)`, `Retail Price`, `Retail Price (USD)`, `Dealer Markup w/o Expenses (%)`, … параметры …, `Dealer Markup w/ Expenses (%)` |
| `dealer` | `Name`, `Dealer Price`, `Dealer Price (USD)`, `Retail Price`, `Retail Price (USD)`, `Dealer Markup w/o Expenses (%)`, … параметры …, `Dealer Markup w/ Expenses (%)` |

Виды колонок (`PricelistColumnKind`): `name`, `editable` (редактируемая цена в исходной валюте), `usd` (та же цена в USD — **редактируемый инпут**, конвертация на фронте; read-only только в `dealer` scope), `markup` (производная наценка, read-only, приглушённый цвет), `statusSummary` (сводка дилерского статуса), `parameter` (динамическая колонка-параметр).
...
**Колонки наценки** (`kind: "markup"`, считаются из USD-значений, поле `markup: MarkupBasis`). Markup-ячейка несёт единицу **и в заголовке, и в ячейке**: показывает округлённое число со знаком `%` (`formatMarkupValue` → `"31%"`). USD-ячейка — редактируемый инпут с приклеенным суффиксом `USD` (read-only в `dealer` scope показывает `formatUsdValue` → `"13,990 USD"`). Параметр-колонки остаются без единицы (она в их лейбле):

| Колонка | id | `markup` | Где | База | Наценка |
|---------|-----|---------|-----|------|---------|
| `Global Markup (%)` | `dealerMarkup` | `dealer` | `supplier` (после `Dealer Price (USD)`) + раскрытие Global-строки по регионам | Plant Price (USD) | наценка на Plant Price, включённая в Dealer Price |
...
- **USD-колонки** — это **редактируемое представление той же цены в долларах**, считается на фронте: `toUsd(amount, currency)` по статичному курсу `CURRENCY_USD_RATE` (зафиксирован как demo-константа, не обновляется в рантайме). Второе значение **не хранится** — источник истины один: цена в исходной валюте. Правка USD-инпута конвертирует ввод обратно (`fromUsd`, округление до целого) и пишет в тот же cell-id, что и колонка исходной валюты. Поэтому цену можно менять в любой из двух валют — обе колонки синхронизируются мгновенно (без скелетона/бэкенда). Компонент `pricelist-price-usd-cell.tsx`; в `dealer` scope колонка read-only (`formatUsdValue`). Presence USD-инпута висит на отдельном id `{cellId}:usd`, чтобы бейдж редактора не подсвечивал инпут исходной валюты.
- **Markup** — производная колонка (тоже приглушённая): наценка дилерской цены над закупочной в процентах, считается из USD-значений (`computeMarkupPercent(purchaseUsd, dealerUsd)`), поэтому валюты сокращаются. `null` (показывается `—`), если закупочная цена пустая или ≤ 0. Тоже идёт через бэкенд-кэш.
...
> Изменение одного входа пересчитывает все зависимые targets: например, правка `Plant Price` (в любой из двух валют) мгновенно обновляет `Plant Price (USD)` на фронте **и** триггерит пересчёт `Global Markup` через бэкенд. Правка base параметра `Total Expenses` пересчитывает унаследованные ячейки колонки и `Dealer Markup w/ Expenses`.
...
src/components/store/pim/pricelists/
  ...
  pricelist-price-cell.tsx                   # редактируемая ячейка цены (исходная валюта + селектор валюты)
  pricelist-price-usd-cell.tsx               # редактируемая ячейка цены в USD (конвертация на фронте → исходная цена)
...
- **Курсы валют статичны** (demo-константа). USD-колонки — редактируемое представление той же цены: хранится только цена в исходной валюте, USD конвертируется на фронте (`toUsd`/`fromUsd`) и пишет обратно в тот же cell-id.
