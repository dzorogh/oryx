...
Заказ на производство — план потребности, не место остатка. Создание и правка плана не пишут журнал склада. «Свободно» = план − назначено − выпущено; «В резерве» = нескладское назначение заказу клиента (проведённый Reservation с местом `production_order` без проводок журнала). «Зарезервировать» на карточке PO создаёт такой резерв и уменьшает свободную потребность. Закрытие снимает невыполненную потребность и **не** списывает складской остаток. Выпуск кладёт товар сразу на склад завода-изготовителя. Статусы (черновик / план / в работе / готов) — workflow; `done` остаётся операционным до явного закрытия. Карточка заказа на производство больше не предлагает «Отменить»: единственный terminal-переход — подтверждённое «Закрыть заказ». Текст закрытия: «Закрытие снимет невыполненную потребность. Складской остаток не списывается. Завершённые выпуски и связанные документы не отменяются.» Выпуск создаётся атомарно через `store_create_production_output` с `p_request_key`. Остатки двигаются только при Done. «Под заказ клиента» на выпуске резервирует уже полученный складской остаток отдельным RSV.
