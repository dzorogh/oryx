...
3. У заказа клиента полоса этапов («Производство → Выпуски → Перемещения → Отгрузки») — отдельная карточка под шапкой; «Связанные» раскрывает резервы и возвраты.
...
- строка выпуска в статусе `draft` / `in_progress` с владельцем «заказ клиента» — плановый «резерв в выпуске»;
...
- закрытие или отмена заказа на производство автоматически отменяет его выпуски в `draft` / `in_progress` (их резервы исчезают); завершённые выпуски не трогаются.

Сценарии «создать заказ на производство под заказ клиента» сразу создают черновик выпуска на весь объём с резервом. Дополнительный резерв — в новый черновик выпуска или в свободное количество имеющегося черновика (`store_reserve_in_production_output`).
