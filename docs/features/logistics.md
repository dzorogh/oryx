...
Сценарии «создать заказ на производство под заказ клиента» сразу создают черновик выпуска на весь объём с резервом. Дополнительный резерв — в новый черновик выпуска или в свободное количество имеющегося черновика (`store_reserve_in_production_output`). Снятие резерва в черновике — обратный перенос в «Свободно» той же операцией `store_move_in_production_output` (перераспределение между владельцами строк черновика): пункт «Снять в выпуске OUT-N» в меню строки заказа клиента и кнопка «Снять» в строке резерва на вкладке «Товары» заказа на производство. Выпуск в работе и завершённый не меняются.
