...
LOCALE_FILE = DEFAULT_FILE.parent / "brain-methods.ru.json"
...
def _load_locale(file: Path = LOCALE_FILE) -> dict[str, dict[str, str]]:
    """Load optional Russian display copy without changing canonical catalog identifiers."""
    try:
...
_TECH_RU = _load_locale()

CATEGORY_LABELS = {
    "collaborative": "Совместные",
    "creative": "Творческие",
    "deep": "Глубокий анализ",
    "introspective_delight": "Рефлексия и личное",
    "structured": "Структурированные",
    "theatrical": "Ролевые и театральные",
    "wild": "Смелые и игровые",
    "biomimetic": "Природные аналогии",
    "quantum": "Квантовые метафоры",
    "cultural": "Культурные",
    "absurdist": "Абсурдистские",
    "constraint": "Жёсткие ограничения",
    "speculative_future": "Сценарии будущего",
}


def category_label(cat: str) -> str:
    """Human-facing Russian category label; custom categories keep their own name."""
    return CATEGORY_LABELS.get(cat, pretty(cat))


def technique_copy(row: dict) -> tuple[str, str]:
    """Russian card copy with a safe fallback for custom and untranslated techniques."""
    translated = _TECH_RU.get(row["technique_name"], {})
    return (
        translated.get("name") or row["technique_name"],
        translated.get("description") or row["description"],
    )
...
<title>Конструктор мозгового штурма BMad</title>
<script>
...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  .sub { margin:0 0 12px; color:var(--muted); font-size:14px; max-width:96ch; }
  ...
  .grp.picks { flex-wrap:nowrap; }
  .glabel { font-size:11px; text-transform:uppercase; letter-spacing:.04em; color:var(--muted); min-width:68px; flex:none; white-space:nowrap; }
  .modes { display:inline-flex; background:var(--control); border-radius:9px; padding:3px; gap:2px; flex:none; }
  .mode { padding:7px 12px; font-size:14px; font-weight:600; color:var(--muted); background:transparent; white-space:nowrap; }
  .mode.on { background:var(--raised); color:var(--accent-ink); box-shadow:0 1px 3px var(--shadow); }
  .modehint { flex:1 1 200px; min-width:0; font-size:13px; color:var(--muted); font-style:italic; }
  .pill { font-size:12px; color:var(--muted); background:var(--control); padding:5px 10px; border-radius:20px; white-space:nowrap; flex:none; }
  .pill b { color:var(--accent-ink); }
  .step { display:inline-flex; align-items:center; gap:6px; font-size:12px; color:var(--ink); background:var(--control2); padding:3px 5px 3px 10px; border-radius:20px; white-space:nowrap; flex:none; }
  .step b { min-width:12px; text-align:center; font-size:13px; color:var(--ink); }
  .step button { width:22px; height:22px; border-radius:50%; background:var(--raised); color:var(--muted); font-size:16px; line-height:20px; text-align:center; box-shadow:0 1px 2px var(--shadow); }
  .step button:hover { color:var(--accent-ink); }
  .total { font-size:12px; color:var(--muted); white-space:nowrap; flex:none; }
  .total.warn { color:var(--warn); font-weight:600; }
  .bar { display:flex; gap:10px 14px; align-items:center; flex-wrap:wrap; }
  .helper { margin:-2px 0 2px 74px; color:var(--muted); font-size:12px; }
  #copy { margin-left:auto; padding:8px 16px; background:var(--accent); color:#fff; font-size:13px; font-weight:700; white-space:nowrap; flex:none; }
  ...
  .goal { font-size:12px; padding:5px 12px; border-radius:16px; background:var(--control); color:var(--muted); font-weight:600; white-space:nowrap; }
  ...
  @media (max-width:720px) {
    ...
    .glabel { min-width:100%; }
    .modes { width:100%; }
    .mode { flex:1; padding-left:7px; padding-right:7px; }
    .grp.picks { flex-wrap:wrap; }
    .modehint, .helper { margin-left:0; }
...
<header>
  <div class="hwrap">
  <div class="titlerow">
    <h1>Конструктор мозгового штурма</h1>
    <button id="theme" class="themebtn" type="button" aria-label="Переключить цветовую тему" title="Переключить цветовую тему"></button>
  </div>
  <p class="sub"><strong>Как начать:</strong> 1) выберите роль ИИ, 2) отметьте 3–4 техники или доверьте подбор ИИ, 3) скопируйте задание и вставьте его в чат. В каталоге {{TOTAL}}</p>

  <div class="composer">
    <div class="grp">
      <span class="glabel">1. Роль ИИ</span>
      <div class="modes" id="modes">
        <button type="button" class="mode on" data-mode="Facilitator">Ведущий</button>
        <button type="button" class="mode" data-mode="Creative Partner">Соавтор</button>
        <button type="button" class="mode" data-mode="Ideate for me">Сделать за меня</button>
      </div>
      <span class="modehint" id="modehint"></span>
    </div>
    <div class="grp picks">
      <span class="glabel">2. Техники</span>
      <span class="pill" title="Карточки, которые вы отметили ниже">Вручную: <b id="pickN">0</b></span>
      <span class="step" title="ИИ случайно добавит техники из каталога">Случайно <button type="button" data-step="rand" data-d="-1" aria-label="Уменьшить число случайных техник">&minus;</button><b id="randN">0</b><button type="button" data-step="rand" data-d="1" aria-label="Увеличить число случайных техник">+</button></span>
      <span class="step" title="ИИ придумает новые техники специально для этой сессии">Новых <button type="button" data-step="inv" data-d="-1" aria-label="Уменьшить число новых техник">&minus;</button><b id="invN">0</b><button type="button" data-step="inv" data-d="1" aria-label="Увеличить число новых техник">+</button></span>
      <span class="step" title="ИИ подберёт техники, которые лучше всего подходят вашей цели">Подбор ИИ <button type="button" data-step="ai" data-d="-1" aria-label="Уменьшить число техник, выбранных ИИ">&minus;</button><b id="aiN">0</b><button type="button" data-step="ai" data-d="1" aria-label="Увеличить число техник, выбранных ИИ">+</button></span>
      <span class="total" id="total" title="Рекомендуем 3–4 техники на сессию">Всего: 0 &middot; 3&ndash;4</span>
      <button id="copy" type="button">3. Скопировать</button>
    </div>
    <p class="helper">Можно сочетать варианты: например, выбрать две карточки вручную и одну доверить ИИ.</p>
  </div>

  {{GOALBAR}}
  <div class="bar">
    <span class="glabel">Категории</span>
    <div class="chips" id="chips">{{CHIPS}}</div>
  </div>

  <div class="banner" id="banner">&#10003; Задание скопировано. Вставьте его в чат, чтобы начать.</div>
  </div>
</header>
<main>
{{BODY}}
</main>
<footer>BMad Method &middot; Мозговой штурм</footer>
<script>
(function(){
  ...
  var MODE_HINTS = {
    'Facilitator': 'ИИ только задаёт вопросы и помогает раскрыть ваши идеи, но не предлагает свои.',
    'Creative Partner': 'Вы и ИИ придумываете вместе: он ведёт сессию и предлагает собственные идеи.',
    'Ideate for me': 'ИИ самостоятельно проведёт весь штурм, подготовит идеи и покажет готовый результат.'
  };
  ...
  function flash(ok, text){
    ...
    b.innerHTML = ok
      ? '✓ Задание скопировано. Вставьте его в чат, чтобы начать.'
      : '⚠ Не удалось открыть буфер обмена. Скопируйте текст из окна и вставьте его в чат.';
    ...
    if (!ok){ window.prompt('Скопируйте этот текст и вставьте его в чат:', text); }
  }
...
CLASSIC_GROUP = "Проверенные профессиональные"
...
CATEGORY_GROUPS = (
    ("Структура и анализ", ("structured", "deep")),
    ("Творчество и новые идеи", ("creative", "biomimetic", "cultural", "speculative_future", "quantum")),
    ("Игра и смелые эксперименты", ("wild", "absurdist", "theatrical", "constraint")),
    ("Рефлексия и совместная работа", ("introspective_delight", "collaborative")),
)

# Human labels for the `good_for` goal tags; this dict's order is the filter-bar order.
GOAL_LABELS = {
    "feature": "Разработать функцию",
    "novel": "Найти новую концепцию",
    "strategy": "Продумать стратегию",
    "planning": "Составить план",
    "diagnosis": "Найти причину проблемы",
    "personal": "Принять личное решение",
    "unstuck": "Выйти из тупика",
}

# Compact filter-bar labels so the sticky header stays one row; cards keep GOAL_LABELS.
GOAL_FILTER_LABELS = {
    "feature": "Функция",
    "novel": "Концепция",
    "strategy": "Стратегия",
...
AUDIENCE_LABELS = {
    "solo": "индивидуально",
    "group": "в группе",
    "either": "одному или в группе",
}


def _fit_label(row: dict) -> str:
    goals = [GOAL_LABELS.get(g, g) for g in row.get("good_for", "").split("|") if g]
    parts = [("Подходит: " + " · ".join(goals))] if goals else []
    audience = AUDIENCE_LABELS.get(row.get("audience", ""))
    if audience:
        parts.append("Формат: " + audience)
    return " · ".join(parts)
...
def _card(r: dict, lead: bool = False) -> str:
    ...
    display_name, display_desc = technique_copy(r)
    display_name = html.escape(display_name)
    display_desc = html.escape(display_desc)
    ...
    return (
        ...
        f'<span><span class="n">{display_name}</span><span class="d">{display_desc}</span>{gf_html}</span></label>'
    )


def _invent_card(canonical_cat: str, display_cat: str, glyph: str) -> str:
    """A dashed 'invent on the fly, in this category's spirit' card appended to each section."""
    return (
        f'<label class="tech invent"><input type="checkbox" data-invent="{canonical_cat}">'
        f'<span class="ic2">{_svg(glyph)}</span>'
        f'<span><span class="n">✨ Придумать новую технику</span>'
        f'<span class="d">ИИ создаст новый метод в духе категории «{display_cat}» специально для этой сессии.</span></span></label>'
    )
...
def html_doc(rows: list[dict]) -> str:
    ...
    def add_section(cat: str) -> None:
        ...
        canonical = html.escape(pretty(cat))
        disp = html.escape(category_label(cat))
        cards = [_card(r) for r in groups[cat]]
        cards.append(_invent_card(canonical, disp, glyph))
        chips.append(f'<button type="button" class="chip" data-cat="{html.escape(cat)}" style="--cc:{hue}">{disp}</button>')
        body.append(
            f'<section data-cat="{html.escape(cat)}" style="--c:{hue}"><h2>{disp}<span class="cnt">{len(groups[cat])}</span></h2>'
    ...
    if classics:
        ...
        chips.append(f'<button type="button" class="chip" data-cat="__classic" style="--cc:{LEAD_HUE}">{disp}</button>')
        body.append(
            f'<section data-cat="__classic" style="--c:{LEAD_HUE}"><h2>{disp}<span class="cnt">{len(classics)}</span></h2>'
    ...
    if leftover:
        body.append('<h2 class="grouphdr" style="--c:#8a8f9e">Другие категории</h2>')
        for c in leftover:
            add_section(c)
    ...
    if present_goals:
        ...
        gchips = "".join(
            f'<button type="button" class="goal" data-goal="{html.escape(g)}" title="{html.escape(GOAL_LABELS.get(g, g))}">{html.escape(GOAL_FILTER_LABELS.get(g, GOAL_LABELS.get(g, g)))}</button>'
            for g in ordered
        )
        goalbar = f'<div class="bar"><span class="glabel">Цель</span><div class="goals" id="goals">{gchips}</div></div>'

    total = html.escape(f"{len(rows)} техник в {len(groups)} категориях.")
    return (
