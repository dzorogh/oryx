...
def test_html_writes_selection_page(lib, tmp_path):
    ...
    assert "Конструктор мозгового штурма" in doc
    assert "1. Роль ИИ" in doc
    assert "3. Скопировать" in doc
    assert "Facilitation mode:" in doc  # copied prompt keeps the parser's canonical protocol
...
def test_russian_locale_covers_shipped_catalog():
    rows = brain.load(brain.DEFAULT_FILE)
    assert set(brain._TECH_RU) == {r["technique_name"] for r in rows}
    assert all(entry.get("name") and entry.get("description") for entry in brain._TECH_RU.values())


def test_russian_category_labels_cover_shipped_catalog():
    rows = brain.load(brain.DEFAULT_FILE)
    assert {r["category"] for r in rows} <= set(brain.CATEGORY_LABELS)
